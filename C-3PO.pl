...
# Add plugin directories to load initial modules.
...
sub main{
	...
	if (defined $options->{'serverFolder'}){
		...
		require Utils::Config;
		my @a= Utils::Config::expandINC($serverFolder);
		unshift @INC, @a;
	...
	# Add plugin directories on top.
	unshift @INC, $lib;
	unshift @INC, $C3PODir;

	my @inc3=();
	for my $i (@INC){addToArray($i,\@inc3);}
	...
	Plugins::C3PO::Logger::debugMessage('inc3'.Data::Dump::dump(@inc3));
