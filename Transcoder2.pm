...
#####################################################################
# Transcoding flow control - Double check when adding support for codecs.
#####################################################################
...
sub useFaadToTranscode{

	# decode to $outcodec when trascoding at the end.
	
	my $transcodeTable =shift;
	
	if (!defined $transcodeTable->{'pathToFaad'}) {return 0};
...
sub _transcode{
	...
	} elsif (useFaadToTranscode($transcodeTable)){
